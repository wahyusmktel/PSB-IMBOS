...
class OperatorPendaftarController extends Controller
{
    public function index(Request $request)
    {
        ...
        $akunPendaftars = AkunPendaftar::with('pendaftarJenjang.jenjang', 'pendaftarJalur.jalur', 'transaksi', 'hasilSeleksi')
            ...
            ->when($request->has('filter_gelombang'), function ($query) use ($request) {
                return $query->where('gelombang', $request->filter_gelombang);
            })
        ...
        // Kirim hasil ke view
        return view('operator.pendaftar.index', compact('akunPendaftars', 'search', 'jenjangs', 'jalurs'))->with('filter_gelombang', $request->filter_gelombang);
    }
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new PendaftarExport($jenjang, $jalur, $statusPembayaran, $gelombang), $fileName);
    }
