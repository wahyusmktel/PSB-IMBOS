...
class PendaftarFormulirController extends Controller
{
    public function cetakFormulir()
    {
        ...
        if ($jalur && $jalur->jalur && !in_array($jalur->jalur->nama_jalur, ['Jalur Reguler', 'Jalur Alumni'])) {
            if ($berkas->isEmpty()) {
        ...
        try {
            ...
            if ($biodataDiri && $biodataDiri->pas_photo) {

                // Sesuaikan dengan cara kamu menyimpan file:
                // misal di DB hanya simpan "pas_foto/foto.jpg"
                $relativePath = $biodataDiri->pas_photo;

                // kalau kamu simpan pakai disk "public"
                $fullPath = storage_path('app/public/' . $relativePath);

                if (file_exists($fullPath)) {
                    $fotoBase64 = base64_encode(file_get_contents($fullPath));
                    $fotoMime   = mime_content_type($fullPath); // contoh: image/jpeg, image/png
                }
            }
            ...
            \Log::error('Error generate Formulir PDF: ' . $e->getMessage(), [
                'pendaftar_id' => $pendaftar_id,
                'trace'        => $e->getTraceAsString(),
            ]);

            Alert::error(
                'Gagal Mengunduh Formulir',
                'Pastikan Pas Foto pada data diri sudah terupload. Terjadi kesalahan saat membuat file PDF. Silakan coba lagi, dan hubungi admin jika masalah berlanjut.'
            );
