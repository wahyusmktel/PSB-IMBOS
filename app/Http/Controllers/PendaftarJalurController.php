...
class PendaftarJalurController extends Controller
{
    ...
    public function index()
    {
        ...
        // [ADDED] Ambil tingkat jenjang pendaftar (mis. 'SMA' / 'SMA')
        $tingkatJenjang = optional($pendaftarJenjang->jenjang)->tingkat_jenjang;

        // [CHANGED] Ambil jalur aktif, tapi sembunyikan 'Jalur Alumni' jika bukan SMA
        $jalurs = ConfigJalurModel::where('status', true)
            ->when($tingkatJenjang !== 'SMA', function ($q) {
                // Sembunyikan berdasarkan nama (case-insensitive)
                $q->whereRaw('LOWER(nama_jalur) <> LOWER(?)', ['Jalur Alumni']);
                // Jika kamu punya kolom kode/slug alumni, lebih baik pakai itu:
                // $q->where('kode', '<>', 'alumni');
            })
            ->get();
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            // [ADDED] Ambil jalur yang dipilih untuk cek nama/kode
            $jalurDipilih = ConfigJalurModel::findOrFail($request->jalur_id);

            // [ADDED] Business rule: Jalur Alumni hanya untuk SMA
            if (strcasecmp($jalurDipilih->nama_jalur, 'Jalur Alumni') === 0 && $tingkatJenjang !== 'SMA') {
                Alert::warning('Tidak Memenuhi Syarat', 'Jalur Alumni hanya tersedia untuk pendaftar jenjang SMA.');
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Dapatkan ID pendaftar yang sedang login
            $pendaftarId = Auth::guard('pendaftar')->id();
            // [ADDED] Ambil jenjang pendaftar
            $pendaftarJenjang = PendaftarJenjangModel::where('pendaftar_id', $pendaftarId)->first();
            $tingkatJenjang   = optional($pendaftarJenjang->jenjang)->tingkat_jenjang;

            // [ADDED] Ambil jalur yang dipilih
            $jalurDipilih = ConfigJalurModel::findOrFail($request->jalur_id);
